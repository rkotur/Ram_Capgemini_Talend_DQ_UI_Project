...
public interface SchemaRepository extends JpaRepository<Schema, Long>  {
    ...
    @Query(value="SELECT ROW_NUMBER() OVER() as id, upper(table_name) as name FROM information_schema.tables where upper(Table_catalog) = ?1 Order by table_name", nativeQuery = true)
